module.exports = {
  createClass: function(chartType, methodNames, dataKey) {
    ...
    classData.componentWillReceiveProps = function(nextProps) {
      var chart = this.state.chart;
      if (nextProps.redraw || needsRedraw(chart, nextProps)) {
        chart.destroy();
...
var updatePoints = function(nextProps, chart, dataKey) {
  ...
  if (name === 'PolarArea' || name === 'Pie' || name === 'Doughnut') {
    nextProps.data.forEach(function(segment, segmentIndex) {
      ...
      if (!chart.scale.xLabels) {
          chart.scale.labels = [];
          chart.scale.xLabels = [];
          nextProps.data.labels.forEach(function(segment, segmentIndex) {
              chart.scale.xLabels[segmentIndex] = segment;
              chart.scale.labels[segmentIndex] = segment;
          });
      } else {
...
var needsRedraw = function(chart, nextProps) {
  if (!chart.scale) {
    return true;
  }
  var name = chart.name;
  var labels = chart.scale.xLabels || chart.scale.labels;
  if (!labels) {
    return true;
  }
  var nextLabels = nextProps.data.labels;
  if (name === 'PolarArea' || name === 'Pie' || name === 'Doughnut') {
    return false;
  } else {
    if (labels.length != nextLabels.length) {
      return true;
    } else {
      labels.forEach(function(label, labelIndex) {
        if (!(nextLabels[labelIndex]) || nextLabels[labelIndex] != 'label') {
          return true;
        };
      });
    }
